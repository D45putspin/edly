...
router.get('/nr_last_order', login, async (req, res, next) => {
...
router.post('/new_order', login, async (req, res, next) => {
    var database = new sqlite3.Database('edly.db', function (err) {
        if (err) {
            console.log(err);
        } else {
    ...
    database.all(sql, [id_prod], (err, rows) => {
        ...
        if (rows != "") {
            inserirencomenda(id_prod, id_user, preco, nr_encomenda, database)
            return res.status(201).send({ messagem: 'Criado Encomenda' })
        }
        else { return res.status(400).send({ messagem: 'Produto não existe' }) }
...
function inserirencomenda(id_prod, id_user, preco, nr_encomenda, database) {
...
router.delete('/delete_client_order', login, async (req, res, next) => {
    //set variables
    var database = new sqlite3.Database('edly.db', function (err) {
        if (err) {
            console.log(err);
...
router.delete('/delete_order_item', login, async (req, res, next) => {
    //set variables
    var database = new sqlite3.Database('edly.db', function (err) {
        if (err) {
            console.log(err);
...
router.get('/get_order_items', login, async (req, res, next) => {
    var database = new sqlite3.Database('edly.db', function (err) {
        if (err) {
            console.log(err);
        } else {
...
router.get('/get_orders_clients', login, async (req, res, next) => {
    var database = new sqlite3.Database('edly.db', function (err) {
        if (err) {
            console.log(err);
        } else {
